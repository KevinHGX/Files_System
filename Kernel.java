...
*/
    public static int chgrp(String filename, short gid) throws Exception {
        if (gid < 0) {
            System.out.println(filename + ": Gid aun no a cambiado: Para obtener Acceso gid tiene que ser mayor a -1");
            return 0;
        }
        ...
        if (indexNodeNumber < 0) {
            process.errno = ENOENT;
            System.err.println("[" + filename + "]: Error. File ingresado no existe");
            return -1;
        }
        ...
        if ((process.getUid() == indexNode.getUid()) || (process.getUid() == 0)) {
          ...
          System.out.println("[" + filename + "]: El Gid de File: "+ filename +" a cambiado Exitosamente");
        } else {
          process.errno = EACCES;
          System.err.println("[" + filename + "]: Error. permiso denegado GID no a cambiado Unicamente el propietario o el superusuario puede cambiar el archivo");
          status = -1;
        }
  ...
  public static void initialize()
  {
    ...
    process = new ProcessContext( (short)1 , (short)1 , dir , umask ) ;
    processCount ++ ;
